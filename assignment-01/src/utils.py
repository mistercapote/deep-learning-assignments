...
def plot_samples(samples, part: int):
    ...
    for idx, (err, img, gt, pred) in enumerate(samples):
        ...
        axes[1, idx].imshow(gt, cmap='nipy_spectral', interpolation='nearest')
        axes[1, idx].set_title(f"Gabarito (Instâncias: {len(np.unique(gt))-1})")
        axes[1, idx].axis('off')

        axes[2, idx].imshow(pred, cmap='nipy_spectral', interpolation='nearest')
        axes[2, idx].set_title(f"Predição (Instâncias: {len(np.unique(pred))-1}) | Erro: {err}")
        axes[2, idx].axis('off')
